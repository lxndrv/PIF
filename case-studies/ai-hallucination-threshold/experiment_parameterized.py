#!/usr/bin/env python3
"""
PIF Hallucination Threshold Experiment - Parameterized Version

Run with different load ranges and trial counts to efficiently find threshold.

Usage:
    # Run both models (default)
    python experiment_parameterized.py --start 100 --stop 1000 --step 100 --trials 3

    # Run only GPT-4o-mini
    python experiment_parameterized.py --start 100 --stop 1000 --step 100 --trials 3 --model gpt-4o-mini

    # Run only Gemini 2.0 Flash
    python experiment_parameterized.py --start 100 --stop 1000 --step 100 --trials 3 --model gemini-2.0-flash

    # Run Claude 3.5 Haiku
    python experiment_parameterized.py --start 100 --stop 1000 --step 100 --trials 3 --model claude-3-5-haiku

    # Run Claude Sonnet 4.5
    python experiment_parameterized.py --start 100 --stop 1000 --step 100 --trials 3 --model claude-sonnet-4-5

    # Run Claude Opus 4.5
    python experiment_parameterized.py --start 100 --stop 1000 --step 100 --trials 3 --model claude-opus-4-5
"""
...
def query_claude(system_prompt: str, question: str, model: str) -> dict:
    """Query Claude models with structured JSON output.

    Args:
        system_prompt: System prompt containing facts and instructions
        question: The question to ask about a fact
        model: Model identifier (e.g., 'claude-3-5-haiku-20241022')

    Returns:
        Dict with 'answer' and 'confidence_score' keys
    """
...
def run_experiment(load_range: range, trials_per_load: int, output_dir: Path, models: list[str]) -> pd.DataFrame:
    """Run the full experiment across all load sizes and trials."""
    results = []

    # Model registry mapping names to query functions
    model_registry = {
        "gpt-4o-mini": query_openai,
        "gemini-2.0-flash": query_gemini,
        "claude-3-5-haiku": lambda sys, q: query_claude(sys, q, "claude-3-5-haiku-20241022"),
        "claude-sonnet-4-5": lambda sys, q: query_claude(sys, q, "claude-sonnet-4-5-20250929"),
        "claude-opus-4-5": lambda sys, q: query_claude(sys, q, "claude-opus-4-5-20251101"),
    }

    # Filter to selected models
    selected_models = {name: func for name, func in model_registry.items() if name in models}
    ...
    total_iterations = len(load_range) * trials_per_load * len(selected_models)
    ...
    with tqdm(total=total_iterations, desc="Running trials") as pbar:
        for load_size in load_range:
            for trial in range(trials_per_load):
                ...
                for model_name, query_func in selected_models.items():
                    response = query_func(system_prompt, question)
                    is_correct = check_answer(response, expected_value)
                    ...
                    results.append({
                        ...
                        "had_error": response.get("error", False)
                    })
                    pbar.update(1)
...
def generate_visualizations(df: pd.DataFrame, output_dir: Path):
    ...
    # Position danger zone label at the rightmost x-value
    max_load = df["load_size"].max()
    ax2.text(max_load * 0.97, 0.9, "Danger\nZone", fontsize=9, ha='right', color='darkred', alpha=0.7)
...
def print_summary(df: pd.DataFrame):
    ...
    for model in df["model"].unique():
        ...
        # Accuracy by load range
        load_sizes = sorted(model_df["load_size"].unique())
        if len(load_sizes) > 1:
            min_load = min(load_sizes)
            max_load = max(load_sizes)
            mid_load = load_sizes[len(load_sizes) // 2]

            low_accuracy = model_df[model_df["load_size"] <= mid_load]["is_correct"].mean()
            high_accuracy = model_df[model_df["load_size"] >= mid_load]["is_correct"].mean()

            print(f"  Accuracy (Load ≤ {mid_load}): {low_accuracy:.2%}")
            print(f"  Accuracy (Load ≥ {mid_load}): {high_accuracy:.2%}")
        ...
        # Show load sizes where errors occurred
        error_loads = model_df[model_df["is_correct"] == False]["load_size"].unique()
        if len(error_loads) > 0:
            print(f"  Errors occurred at loads: {sorted(error_loads)}")

    # PIF validation
    print("\n" + "-" * 60)
    print("PIF HYPOTHESIS CHECK:")

    load_sizes = sorted(df["load_size"].unique())
    if len(load_sizes) > 1:
        mid_load = load_sizes[len(load_sizes) // 2]
        overall_low = df[df["load_size"] <= mid_load]["is_correct"].mean()
        overall_high = df[df["load_size"] >= mid_load]["is_correct"].mean()
        ...
        if overall_high < overall_low:
            ...
            print(f"  ✗ Accuracy did not drop ({overall_low:.2%} → {overall_high:.2%})")
            print("  → Threshold not yet reached in this load range")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="PIF Hallucination Threshold Experiment")
    parser.add_argument("--start", type=int, required=True, help="Start load size")
    parser.add_argument("--stop", type=int, required=True, help="Stop load size (exclusive)")
    parser.add_argument("--step", type=int, required=True, help="Load step size")
    parser.add_argument("--trials", type=int, required=True, help="Trials per load")
    parser.add_argument(
        "--model",
        type=str,
        choices=["gpt-4o-mini", "gemini-2.0-flash", "claude-3-5-haiku", "claude-sonnet-4-5", "claude-opus-4-5", "all"],
        default="all",
    ...
    if args.model == "all":
        models_to_run = ["gpt-4o-mini", "gemini-2.0-flash"]
    else:
        models_to_run = [args.model]

    # Check for required API keys based on selected models
    if "gpt-4o-mini" in models_to_run and not os.environ.get("OPENAI_API_KEY"):
        print("ERROR: OPENAI_API_KEY not found in environment.")
        print("Please add it to .env file in the project root.")
        return

    if "gemini-2.0-flash" in models_to_run and not os.environ.get("GEMINI_API_KEY"):
        print("ERROR: GEMINI_API_KEY not found in environment.")
        print("Please add it to .env file in the project root.")
        return

    # Check if any Claude model is being used
    claude_models = ["claude-3-5-haiku", "claude-sonnet-4-5", "claude-opus-4-5"]
    if any(model in models_to_run for model in claude_models) and not os.environ.get("ANTHROPIC_API_KEY"):
        print("ERROR: ANTHROPIC_API_KEY not found in environment.")
        print("Please add it to .env file in the project root.")
        return

    # Create output directory with parameter slug
    base_dir = Path(__file__).parent

    # Include model name in slug if specific model selected
    if args.model == "all":
        dir_name = f"run_load_{args.start}_{args.stop}_step_{args.step}_trials_{args.trials}"
    else:
        dir_name = f"run_load_{args.start}_{args.stop}_step_{args.step}_trials_{args.trials}_{args.model}"
    ...
    # Run experiment
    load_range = range(args.start, args.stop, args.step)
    df = run_experiment(load_range, args.trials, output_dir, models_to_run)
